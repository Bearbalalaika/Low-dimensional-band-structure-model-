...
if nargin > 8 
    warning('There must be at most 8 input agruments unless arguments are passed to a function')
end 

if isstr(blockVectorX)
...
if verbosityLevel 
    ...
    if issparse(blockVectorY) ~= issparse(blockVectorX)
        warning('The sparsity formats of the initial guess and the constraints are inconsistent')
    end
end
...
if isempty(operatorB)
    ...
    [gramXBX,cholFlag]=chol(gramXBX); 
    if  cholFlag ~= 0
        error('The initial approximation after constraints',...
            ' is not full rank or/and operatorB is not positive definite')
    end
    blockVectorX = blockVectorX/gramXBX; 
...
if n-sizeY < 5*blockSize
    ...
    opts.issym=1;
    if isreal(operatorA) & ~isstr(operatorA) 
        sigma='SA';
        opts.isreal=1;
    else
...
%     end
    ...
    if isempty(operatorB)
        ...
        if  cholFlag == 0
            ...
            warning('The residual is not full rank.')
            break
        end
    else    
        ...
        if  cholFlag == 0
            ...
            warning('The residual is not full rank or/and operatorB is not positive definite.')
            break
        end
    ...
    if iterationNumber > 1
        ...
        if isempty(operatorB)
            ...
            if  cholFlag == 0
                ...
                warning('The direction matrix is not full rank.')
                break
            end
        else
            ...
            if  cholFlag == 0
                ...
                warning('The direction matrix is not full rank or/and operatorB is not positive definite.')
                break
            end 
        end
        clear gramPBP
    ...
    for cond_try=1:2,           %cond_try == 2 when restart
        ...
        if (condestG/condestGmean > 2 & condestG > 2 )| condestG > 8 
            ...
            if cond_try == 1 & ~restart 
                ...
                warning('Gramm matrix ill-conditioned: results may be unpredictable')
            end
        else
             break
        end
