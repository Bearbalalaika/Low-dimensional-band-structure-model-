...
function out = schrodQW(z, Al, levels, fields, kxy)
...
if length(Al) == 1;
    ...
    Al_QW = z(2);
    Al_barrier = z(3);
    z = -max(50, round((5*width))):dz:max(round((5*width)), 50);
    %Al = Al_QW*ones(size(z)); Al(find(z<(-width/2))) = Al_barrier; Al(find(z>=(width/2))) = Al_barrier;
    Al = Al_QW*ones(size(z)); 
    Al = 0.001*Al.*(z.^2);
%    Al(find(((-10.5-4.5-width/2)<z)&(z<(-4.5-width/2))))=Al_QW;
 %   Al(find(((10.5+4.5+width/2)>z)&(z>(4.5+width/2))))=Al_QW;
    %Al (find((z<2)&(z>1)))=0.9;
...
figure; hold on;
%Pout.E.WF=[1 2 3].*ones(size(out.E.WF));

%plot(z, out.E.WF);
c='brkbrkbrkb';
for i=1:10
plot(z, out.E.E(i)+out.E.WF(:,i).^2, c(i));
end
%hold on;
plot(z,Al);
