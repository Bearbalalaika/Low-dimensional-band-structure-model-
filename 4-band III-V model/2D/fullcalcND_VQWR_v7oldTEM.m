...
%%%DEFINE POTENTIAL
C = buildVQW(x0, y0, z0, dd, -1, 0);
%Ct = buildTriangle(x0, y0, z0, 42, 0, 0);
[X, Y] = meshgrid(x0, y0); R = sqrt(X.^2+Y.^2); Ct = 0*R; Ct(find(R<dd*2/2)) = 1; %cylindrical crosssection
Ct = circshift(Ct, [0, -3]);
C(find(C==0))  = 0.30;
C(find(C==1))  = 0.17;
C(find(Ct==1)) = 0.04;
...
save(['InfinitelongVQWR_' num2str(dd) 'nm_Diameter_v7oldTEM'], 'Q')